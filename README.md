...
一个个人使用的 Chromium 浏览器扩展，用自己的 AI API 总结视频网站字幕。它模仿了 “AI Summary for YouTube” 的页面面板体验，但把支持范围扩展到了 YouTube、Bilibili、通用 HTML5 视频轨道，以及页面可见的转写文稿。
...
- 转写文稿：没有标准字幕轨道时，优先读取页面里已经展开的 transcript/caption 文本。
...
- 界面国际化：设置页可在中文和 English 之间切换，弹窗、页面面板和后台进度提示会同步更新。
...
5. 如果平台没有暴露字幕轨道，先打开页面自带的转写文稿，再刷新面板读取。
...
- 可见转写兜底：标准字幕提取失败时，会尝试读取页面里已经展开的转写文本。
